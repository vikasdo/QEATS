...
linuxTyp=`cat $1|grep -m 1 'NAME' |awk '{print tolower($0)}'`
linuxType=`echo "${linuxTyp:6:6}" `
...
ubuntuVersion=`cat $1|grep -m 1 'VERSION' `
ubuntuVersion=`echo ${ubuntuVersion:9:5} `
 ubuntuVersion=`echo ${ubuntuVersion%.*}` 
